...
5. 不要关闭终端，**直接**在**当前终端**运行，否则临时变量API会失效
...
```
### 4.操作
位于Live2D窗口时，左键按住可以使模型注视鼠标位置。

中键滚轮调整显示大小，按压并拖动调整窗口内模型位置。

使用聊天功能时需要等待冷启动。时长视网络情况而定。待系统提示初始化完成即可开始对话。

退出可直接按下esc，或使用对话窗口的x，***不要使用Live2D的那个x***

---

### 已知问题：
1. 点击Live2D窗口的x时，测试3次均导致虚拟机直接重启，原理不详。
2. 模型加载，字体选择未调用图形化文件管理器，使用体验不佳。
3. 网络要求较高，但这是由Google AI Studio造成。
